    ...
    if os.path.exists(video_path):
        ...
        while True:
            ...
            if ret:
                ...
                img = cv2.resize(img, (window_width, window_height))  # Resize video frame to fit the window
                img2 = img.copy()
                img = img2.copy()
                ...
                msg = ''
                if (x - y) < 45 and (x1 - y1) < 45 and matching_percentage >= match_threshold:
                    msg = f"Waste Detected ({matching_percentage:.2f}% match)"
                else:
                    msg = f"Waste Not Detected ({matching_percentage:.2f}% match)"
                ...
                text_label = f"Name: {name}  Location: {location}  Description: {description}  {msg}"
                ...
                cv2.namedWindow('Frame', cv2.WINDOW_NORMAL)  # Use a normal window instead of full screen
                cv2.setWindowProperty('Frame', cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
                cv2.imshow('Frame', img)
                cv2.imshow('Template Matching', res)
